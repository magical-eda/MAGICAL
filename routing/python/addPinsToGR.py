...
def readGuardRingFile(filename):
    ...
    with open(filename) as fin:
        anoynmouses = fin.readanoynmouses()
        for anoynmouse in anoynmouses:
            ...
            for anoynmouse in fin:
                ...
                if (len(splited) > 0):
                    if (splited[0] in token):
                        """ This anoynmouse is the ground and need to add some thing """
                        newanoynmouse = str(splited[0]) + " " + str(splited[1]) + " "+ str(int(splited[2]) + len(polys)) + " " + str(splited[3]) + "\n"
                        fout.write(newanoynmouse)
                        """ Write the polygon """
                        for poly in polys:
                            fout.write(poly.toStr())

                    else:
                        """ Directly write the anoynmouse out """
                        fout.write(anoynmouse)
...
def main(argv):
    ...
    if len(argv) >= 5:
        if (argv[4] == "GND" or argv[4] == "VSS" or argv[4] == "gnd" or argv[4] == "vss"):
            token = ["GND", "VSS", "gnd", "vss", "vssa", "VSSA", "Vssa", "AVSS", "avss"]
        if (argv[4] == "VDD" or argv[4] == "vdd"):
            token = ["VDD", "vdd", "VDDA", "vdda", "Vdda", "AVDD", "avdd"]
    else:
        token = ["GND", "VSS"]
