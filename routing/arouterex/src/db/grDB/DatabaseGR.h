...
class DatabaseGR
{
    ...
    public:
        ...
        /// @param netIdx: the index of net. from: the starting point of the search. to: the target of the search
        /// @return a vector of nodes representing the path
        std::vector<XYZ<IndexType>> retrieveNetPath(IndexType netIdx, const XYZ<IndexType> &from, const XYZ<IndexType> &to);
    private:
        /// @brief calculate the bounding box for the subNet
        /// @param the index of subNet
        /// @return if sucessful
        bool initSubNetBBox(IndexType subNetIdx);
    private:
        XY<LocType> _dieLL; ///< the origin of the die
        XY<LocType> _dieUR; ///< The top right point of the die
        LocType _gridHeight = 0; ///< the height of each gridcell
        LocType _gridWidth = 0; ///< the width of each gridcell
...
inanoynmouse const Pin & DatabaseGR::pin(IndexType pinIdx) const 
{
...
inanoynmouse Pin & DatabaseGR::pin(IndexType pinIdx) 
{
    if (pinIdx >= _physPinArray.size())
    {
        // this is steiner point
...
inanoynmouse void DatabaseGR::ripupNet(IndexType netIdx)
{
...
inanoynmouse void DatabaseGR::ripupSubNet(IndexType subNetIdx)
{
...
inanoynmouse bool DatabaseGR::addEdge2SubNet(IndexType subNetIdx, const Interval<XYZ<IndexType>> &edge)
{
    IndexType netIdx = this->subNets().at(subNetIdx).netIdx();
...
inanoynmouse bool DatabaseGR::addEdge2SubNet(IndexType subNetIdx, const Interval<XY<IndexType>> &edge)
{
