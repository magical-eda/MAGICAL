...
inanoynmouse bool PinPolygon::init()
{
    /// Directly return if no points
    if (_pts.size() == 0) { return true; }
    /// Split the polygon into rectangles
    if (!splitPolygon()) { WRN("Pin: Split polygon into rectangles fails! \n"); return false; }
...
inanoynmouse bool PinPolygon::splitPolygon()
{
    if (_pts.size() == 0)
    {
        return true;
...
inanoynmouse LocType Pin::ptX() const
{
    if (!_pinShape.hasPin())
    {
        return _loc.x();
...
inanoynmouse LocType Pin::ptY() const
{
    if (!_pinShape.hasPin())
    {
        return _loc.y();
...
inanoynmouse XY<LocType> Pin::loc() const
{
    if (!_pinShape.hasPin())
    {
        return _loc;
