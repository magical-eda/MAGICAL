//
// Created by Keren on 09/12/2018
//
...
class Database
{
    public:
        ...
        /// @brief get the active region information
        /// @return the active region information
        const ActiveRegionRtree & activeRegion() const { return _activeRegion; }
        /// @brief get the active region information
        /// @return the active region information
        ActiveRegionRtree & activeRegion() { return _activeRegion; }
        ...
        /// @brief get the routing guide
        /// @return the routing guide
        const RouteGuide & routeGuide() const { return _routeGuide; }
        /// @brief get the routing guide
        /// @return the routing guide
        RouteGuide & routeGuide() { return _routeGuide; }
        /// @brief get the routing guide cost
        /// @param first: net index
        /// @param second: x-th grid cell
        /// @param third: y-th grid cell
        /// @return the routing guide cost
        IntType routeGuideCost(IndexType netIdx, IndexType x, IndexType y) const { return _routeGuide.cost(netIdx, x, y); }
        /// @brief get routeGuideCost from to to 
        /// @param first: net index
        /// @param second: location from
        /// @param sthird: location to
        /// @return the routing guide cost
        IntType routeGuideCostPath(IndexType netIdx, XY<LocType> from, XY<LocType> to) const;
        /// @brief get guide confi from to to 
        /// @param first: net index
        /// @param second: location from
        /// @param sthird: location to
        /// @return the routing guide cost, can be negative
        IntType routeGuideConfiCostPath(IndexType netIdx, XY<LocType> from, XY<LocType> to) const;
        /*------------------------------*/ 
        /* Setters                      */
        /*------------------------------*/ 
    ...
    private:
        ...
        RouteGuide _routeGuide; ///< The input routing guide
        ActiveRegionRtree _activeRegion; ///< The interface for querying active region information
};
...
inline IntType Database::routeGuideCostPath(IndexType netIdx, XY<LocType> from, XY<LocType> to) const
{
    CostType guideCost = 0;
    IntType count = 0;
    // Horizontal
    auto fromIdx = _grDB.gridIdx(from);
    auto toIdx = _grDB.gridIdx(to);
    ...
    for (IndexType xId = xLo; xId <= xHi; ++xId)
    {
        for (IndexType yId = yLo; yId <= yHi; ++yId)
        {
            ...
            count++;
           guideCost += _routeGuide.cost(netIdx, xId, yId);
        }
    }
    if (count == 0) { return  0; }
    return guideCost; // / count;
}
inline IntType Database::routeGuideConfiCostPath(IndexType netIdx, XY<LocType> from, XY<LocType> to) const
{
    CostType guideCost = 0;
    IntType count = 0;
    // Horizontal
    ...
    for (IndexType xId = xLo; xId <= xHi; ++xId)
    {
        for (IndexType yId = yLo; yId <= yHi; ++yId)
        {
            ...
            count++;
           guideCost += _routeGuide.guideConfi(netIdx, xId, yId);
        }
    }
    if (count == 0) { return  0; }
    return guideCost / count; // / count;
}
