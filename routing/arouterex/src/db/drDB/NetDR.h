...
class NetDR
{
    public:
        ...
        /// @brief get whether the net is wavied in global routing
        /// @return whether the net is waived in global routing
        ...
        /// @brief set whether this net is waived in global routing
        /// @param whether this net is waived in global routing
        ...
        /* Operations                   */
        /*------------------------------*/ 
        /// @brief remove a empty node. The node should have no parent, sibling or child.
    ...
    private:
        /// @brief rename a node. Change the its parent, child, sibling's pointer to the new id
    ...
    private:
        ...
        bool _waiveGr = false; ///< Whether this net is waived in global routing
};

inline void NetDR::init()
{
    _nodes.resize(1); /// Only one root node
}

inline void NetDR::removeEmptyNetNode(IndexType nodeIdx)
{
    Assert(!this->node(nodeIdx).hasChild());
    Assert(!this->node(nodeIdx).hasParent());
    Assert(!this->node(nodeIdx).hasSibling());
...
inline void NetDR::renameNode(IndexType oldIdx, IndexType newIdx)
{
    if (this->node(oldIdx).hasParent())
    {
        ...
        else
        {
            ...
            Assert(this->node(child).hasSibling());
            while(this->node(child).sibling() != oldIdx)
            {
                child = this->node(child).sibling();
                Assert(node(child).hasSibling());
            }
            this->node(child).setSibling(newIdx);
        }
    }
    // Clear the children
    if (this->node(oldIdx).hasChild())
    {
        ...
        while(this->node(child).hasSibling())
        {
...
inline void NetDR::addChildToParent(IndexType childIdx, IndexType parentIdx)
{
    if (this->node(parentIdx).hasChild())
    {
        // need to find the last child and set its sibling
        IndexType child = this->node(parentIdx).child();
        while(this->node(child).hasSibling())
        {
            child = this->node(child).sibling();
        }
        this->node(child).setSibling(childIdx);
    }
    else 
    {
...
inline IndexType NetDR::addWireNodeFromParent(IndexType parentNodeIdx, const NetNodeLoc &loc, const NetNodeWire &wire)
{
...
inline IndexType NetDR::addFakeNodeFromParent(IndexType parentNodeIdx, const NetNodeLoc &loc)
{
    IndexType nodeIdx = this->numNodes();
    _nodes.emplace_back(NetNode(loc));
    _nodes.back().setFake();
...
inline IndexType NetDR::addViaNodeFromParent(IndexType parentNodeIdx, const NetNodeLoc &loc, const NetNodeVIA &via)
{
    IndexType nodeIdx = this->numNodes();
    _nodes.emplace_back(NetNode(loc));
...
inline IndexType NetDR::addWireNodeFromChild(IndexType childNodeIdx, const NetNodeLoc &loc, const NetNodeWire &wire)
{
    DBG("%s NODE OPT \n", __FUNCTION__);
    IndexType nodeIdx = this->numNodes();
    _nodes.emplace_back(NetNode(loc));
...
inline IndexType NetDR::addViaNodeFromChild(IndexType childNodeIdx, const NetNodeLoc &loc, const NetNodeVIA &via)
{
    DBG("%s NODE OPT \n", __FUNCTION__);
    IndexType nodeIdx = this->numNodes();
    _nodes.emplace_back(NetNode(loc));
...
inline void NetDR::connectWire(IndexType parentNodeIdx, IndexType childNodeIdx, const NetNodeWire &wire)
{
    //DBG("%s NODE OPT \n", __FUNCTION__);
...
inline void NetDR::connectVia(IndexType parentNodeIdx, IndexType childNodeIdx, const NetNodeVIA &via)
{
    DBG("%s NODE OPT \n", __FUNCTION__);
    Assert(!this->node(childNodeIdx).hasParent());
    // Set up the shape
...
/// @brief determine whether the node is existing in the node, and return the index of the node
/// @return INDEX_TYPE_MAX if not existing, the index of the node otherwise
inline IndexType NetDR::existNode(XY<LocType> loc, IndexType layer) const
{
...
/// @brief determine if the input node is somewhere between two connected nodes. Return the parent and child indices
/// @return .first = parent node index, .second = child node index. If INDEX_TYPE_MAX, then not found
inline std::pair<IndexType, IndexType> NetDR::betweenNodes(XY<LocType> loc, IndexType layer) const
{
    for (IndexType nodeIdx = 0; nodeIdx < this->numNodes(); ++nodeIdx)
    {
        // Check every shape
...
inline void NetDR::copySymRouteTree(const NetDR &rhs, const SymNet &symNet)
{
