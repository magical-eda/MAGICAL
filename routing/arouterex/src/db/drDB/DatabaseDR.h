...
namespace DRUtil
{
    ...
    inline IndexType routeLayer2AllLayer(IndexType routingLayerIdx)
    {
    ...
    inline IndexType allLayer2RouteLayer(IndexType allLayerIdx)
    {
        Assert(allLayerIdx % 2 == 0);
        return allLayerIdx / 2;
    }
    /// @brief whether is routable layer
    /// @param the index of layer in all layers
    /// @return whether is routable layer
    inline bool allLayerIsRouteLayer(IndexType allLayerIdx)
    {
        return allLayerIdx % 2 == 0;
    }
    ...
    inline IndexType routeLayer2CutLayer(IndexType lowerMetalLayer)
    {
    ...
    inline std::pair<std::pair<LocType, LocType>, std::pair<LocType, LocType>> spacingCheckBundle(const Box<LocType> &lhs, const Box<LocType> &rhs)
    {
        LocType paraLen = 0;
        LocType spacing = 0;
        LocType width1 = 0;
