...
inline bool RoughGridNode::netInGlobalRoutingGuide(IndexType netIdx) const 
{
    return std::binary_search(_netGR.begin(), _netGR.end(), netIdx);
}

inline bool RoughGridNode::netInGlobalRoutingPenalty(IndexType netIdx) const
{
    return std::binary_search(_netGRPenalty.begin(), _netGRPenalty.end(), netIdx);
}

inline void RoughGridNode::addGlobalRoutingPenalty(IndexType netIdx)
{
    if (!netInGlobalRoutingGuide(netIdx))
    {
        
        // using find and iterator may be dangerous (not sure?)
        /// Use linear search, and maintain the ordering
        IndexType idx = 0;
...
inline void RoughGridNode::addGlobalRoutingGuide(IndexType netIdx)
{
    // using find and iterator may be dangerous (not sure?)
    /// Use linear search, and maintain the ordering
    IndexType idx = 0;
...
inline void RoughGridNode::addGlobalRoutingGuideSafe(IndexType netIdx)
{
...
class RoughGrid
{
    public:
        ...
        /// @brief min tile height width
        /// @return the average of width and height
        LocType                                         tileAvg() const                                     { return (_tileWidth + _tileHeight) / 2; }
...
inline RoughGrid::RoughGrid(const Box<LocType> &dieBox, LocType nodeWidth, LocType nodeHeight, IndexType numLayers)
{
...
inline Box<LocType> RoughGrid::nodeBox(IndexType x, IndexType y) const 
{
...
inline XY<IndexType> RoughGrid::nodeIdx(LocType x, LocType y) const
{
    auto xy = XY<IndexType> ((x - _dieBox.xLo()) / _tileWidth, ( y -_dieBox.yLo()) / _tileHeight);
    if (xy.x() >= sizeX())
    {
        xy.setX( sizeX() - 1);
    }
...
inline bool RoughGrid::netInGlobalRoutingGuide(IndexType netIdx, XY<LocType> loc, IndexType layer) const
{
...
inline bool RoughGrid::netInGlobalRoutingPenalty(IndexType netIdx, XY<LocType> loc, IndexType layer) const
{
    if (layer >= this->numLayers())
    {
        return false;
...
inline void RoughGrid::addLocationToGlobalRouting(IndexType netIdx, XY<LocType> loc, IndexType layer)
{
