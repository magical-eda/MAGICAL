...
CommandLineOptions::CommandLineOptions(int argc, char** argv)
{
    ...
    _parser.add <std::string> ("routeGuide", '\0', "Routing guide configuration", false);
    ...
    _parser.add <std::string> ("techLef", 'l', "technology LEF", false);
    _parser.add <std::string> ("techfile", 't', "techfile including layer ID", false);
    _parser.add <std::string> ("simpleTech", '\0', "Simple technology file", false);
...
void CommandLineOptions::populateAndCheckOptions()
{
    ...
    simpleTech      = _parser.get<std::string>("simpleTech");
...
ProgArgs parseProgArgsCMD(IntType argc, char** argv)
{
    ...
    progArgs.setRouteGuideConfig(opt.routeGuideConfig);
    progArgs.setLogFile(opt.log);
    progArgs.setOutputFile(opt.outputFile);
    progArgs.setSimpleTechFile(opt.simpleTech);
