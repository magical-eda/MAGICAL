...
bool ARouter::read()
{
    if (_db.progArgs().isTechLEF() && _db.progArgs().isTechFileSet())
    {
        // tech LEF
    ...
    else if (_db.progArgs().isSimpleTechSet())
    {
        // Simple tech
        
        if (!ParseSimpleTech(_db.techDB(), _db.macroDB()).parse(_db.progArgs().simpleTech()))
        {
            return false;
        }
    }
    ...
    {
        ERR("ARouter::%s: no tech-related file was set \n", __FUNCTION__);
        return false;
    }
    // Design benchmark
    ...
    {
        ParserGuide(_db).parse(_db.progArgs().routeGuideConfig());
    }
    else
    {
...
bool ARouter::preprocessing()
{
    ...
    {
        ERR("Processing symmetry net configuration file failed!\n");
        return false;
    }
    // Self sym
    if (!ProcessSelfSymNet(_db).process())
    {
        ERR("Processing self symmetry net configuration file failed!\n");
        return false;
    }
    // Blockages
    if (!ProcessBlockage(_db).process())
    {
        ERR("Processing blockage failed! \n");
        return false;
    }
    return true;
}
...
bool ARouter::writeGDS()
{
    ...
    if (_db.progArgs().isPlacementLayoutFileSet())
    {
        //WriteGdsLayoutDb(_db).debugWriteNet(6, "debug/crossn");
        //WriteGdsLayoutDb(_db).debugWriteNet(11, "debug/vdd");
        //WriteGdsLayoutDb(_db).debugWriteNetArray({6, 11}, "debug/short");
