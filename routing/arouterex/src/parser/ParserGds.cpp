...
bool ParserGds::readPlacementLayout(const std::string &fileName)
{
    ...
    // Read in Active region
    for (const auto &object : flatCell.objects())
    {
        ::GdsParser::GdsDB::GdsObjectHelpers()(object.first, object.second, ExtractActiveRegionAction(TSMC40_OD_LAYER, _activeRegions));
    }
...
void ParserGds::scaleDesign()
{
    ...
    for (auto &od : _activeRegions)
    {
        od.scale(scale, scale);
    }
...
{
    Box<LocType> dieBox = Box<LocType>(LOC_TYPE_MAX, LOC_TYPE_MAX, LOC_TYPE_MIN, LOC_TYPE_MIN);
    // Get poblks
    ...
    // Convert the ODs
    std::vector<Box<LocType>> ods;
    for (auto &od : _activeRegions)
    {
        ::klib::convertPolygon2Rects<LocType>(od.pts, ods);
    }
    _db.activeRegion().init(ods);

    // Override the die region
    INF("PaserGds: override the die box as %s \n", dieBox.toStr().c_str());
    _db.grDB().dieLL() = dieBox.ll();
    _db.grDB().dieUR() = dieBox.ur();
    return true;
}
