...
bool ParserIspd08::readFilePinShapeBenchmark()
{
    ...
    for (IndexType netIdx = 0; netIdx < _rawDB.netSize(); ++netIdx)
    {
        ...
        ss >> minimumWidth;
        ISPDRawDb::ispdNet tempNet = ISPDRawDb::ispdNet(netName, netIdxx, pinSize, minimumWidth );
    ...
    // Set die boundary
    _rawDB.setXLF(dieXLo * _scale);
    _rawDB.setYLF(dieYLo * _scale);
    _rawDB.setXHi(dieXHi * _scale);
    _rawDB.setYHi(dieYHi * _scale);
...
bool ParserIspd08::trans2DB()
{
    ...
    for (IndexType netIdx = 0; netIdx < _rawDB.netSize(); ++netIdx)
    {
        ...
        temp.setWeight(net._minimumWidth);
