...
class GrDB_v2_to_DrDB_v2_converter
{
    ...
    private:
        ...
        /// @return if the conversion is successful
        bool convertRoutingGuide(IndexType netIdx, NetDR &drNet);
        /// @brief convert routing guide 2D global routing
        /// @param netIdx: the index of the net in the net array.
        /// @return if the conversion is successful
        bool convertRoutingGuide2D(IndexType netIdx);
        /// @brief Enable routing guide with pins
        /// @return if successful
        bool convertRoutingPin();
        /// @brief Enable routing guide with pins
        /// @return if successful
        bool convertRoutingPin2D();

        void addPenaltyNearyby(const XYZ<IndexType> &node, IndexType netIdx);
