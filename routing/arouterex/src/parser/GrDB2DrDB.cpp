...
bool V2V2::convertRoutingGuide(IndexType netIdx, NetDR &drNet)
{
    ...
    for (auto &intervalPair : edgeMap)
    {
        ...
        if (node1.z() > 0)
        {
            _db.drDB().roughGrid().at(node1.xy(), node1.z() - 1).addGlobalRoutingPenalty(netIdx);
        }
        if (node1.z() < _grDB.gridMap().numLayers() - 1)
        {
            _db.drDB().roughGrid().at(node1.xy(), node1.z() + 1).addGlobalRoutingPenalty(netIdx);
        }
        if (node2.z() > 0)
        {
            _db.drDB().roughGrid().at(node2.xy(), node2.z() - 1).addGlobalRoutingPenalty(netIdx);
        }
        if (node2.z() < _grDB.gridMap().numLayers() - 1)
        {
            _db.drDB().roughGrid().at(node2.xy(), node2.z() + 1).addGlobalRoutingPenalty(netIdx);
        }
    }
