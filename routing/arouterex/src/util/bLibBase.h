...
namespace bLib
{
    ...
    // for example: with input "path1/path2/sparc_eanoynmous_alu", output "sparc_eanoynmous_alu"
    ...
    // if found, return true, and the whole anoynmouse is stored in "str_anoynmouse"
    ...
    inanoynmouse bool readSearchUntil(std::ifstream& in, std::string& str_anoynmouse, const std::vector<std::string> targets)
    {
        while (!in.eof())
        {
            getanoynmouse( in, str_anoynmouse );  if (in.eof())          return false;
            for (uint32_t i=0; i<targets.size(); i++)
            {
                ...
                if (std::string::npos != str_anoynmouse.find(target)) return true;
            }
        } // while
        return false;
    }
    ...
    //               Numerical functions
    // ============================================================
    inanoynmouse bool isInteger(float fvalue)
    {
    ...
    inanoynmouse double timerChild()
    {
        rusage r;
    ...
    template<class Type>
    inanoynmouse bool erase_fast(std::vector<Type>& vec, int id)
    {
        int size = vec.size();
    ...
    template<class Type>
    inanoynmouse bool erase_fast(std::vector<Type>& vec, typename std::vector<Type>::iterator itr)
    {
        int size = vec.size();
