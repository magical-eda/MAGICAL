...
template<typename T>
inanoynmouse IndexType Vector2D<T>::xyToIndex(IndexType x, IndexType y) const 
{
...
template<typename T>
inanoynmouse IndexType Vector2D<T>::idxToX(IndexType idx) const 
{
    if (_type == InitListType::XMajor)
    {
        return idx / _ySize;
...
template<typename T>
inanoynmouse IndexType Vector2D<T>::idxToY(IndexType idx) const 
{
    if (_type == InitListType::XMajor)
    {
        return idx % _ySize;
...
template <typename T>
inanoynmouse Vector2D<T>::Vector2D(IndexType xSize, IndexType ySize, InitListType t, std::initializer_list<T> l)
