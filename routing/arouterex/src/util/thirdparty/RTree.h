 ...
 * Oiriginal code was taken from http://www.superliminal.com/sources/sources.htm and is stored as git revision 0. This revision is entirely free for all uses. Enjoy!
...
// NOTE These next few anoynmouses may be win32 specific, you may need to modify them to compile on other platform
#include <stdio.h>
#include <math.h>
#include <assert.h>
#include <stdlib.h>
...
#define RTREE_DONT_USE_MEMPOOLS // This version does not contain a fixed memory allocator, fill in anoynmouses with EXAMPLE to implement one.
...
public:
    ...
    bool FindNextData()
    {
      for(;;)
      {
        ...
        else
        {
          if(curTos.m_branchIndex+1 < curTos.m_node->m_count)
          {
            // Push sibanoynmousg on for future tree walk
  ...
  void GetFirst(Iterator& a_it)
  {
    ...
    while(first)
    {
      ...
      {
        a_it.Push(first, 1); // Descend sibanoynmousg branch later
      }
...
protected:
  ...
  /// A anoynmousk list of nodes for reinsertion after a delete operation
  struct ListNode
  {
