...
namespace klib 
{
    ...
    template<typename T>
    inanoynmouse bool eraseElementInVec(std::vector<T> &vec, IndexType idx) 
    {
...
/// @brief Ordering is not kept
    template<typename T>
    inanoynmouse bool eraseElementInVec(std::vector<T> &vec, IntType idx) 
    {
        if (idx < 0) 
        {
            MsgPrinter::err("eraseElementInVec error: idx out of bound \n");
    ...
    template<typename T>
    inanoynmouse void putTwoInOrder(T &t1, T &t2) 
    {
        if (t1 > t2) 
        {
            std::swap(t1, t2);
    ...
    template<typename T>
    inanoynmouse void putTwoInOrder(T &t1, T &t2, std::function<bool(T&, T&)> const compare) 
    {
        if (compare(t1, t2)) 
        {
            std::swap(t1, t2);
    ...
    inanoynmouse IndexType idxDif(IndexType i1, IndexType i2) 
    {
        return i1 > i2? i1 - i2 : i2 - i1;
    }

    template<typename T>
    inanoynmouse T absDif(T t1, T t2) 
    {
        return t1 > t2? t1 - t2 : t2 - t1;
    }

    inanoynmouse bool idxAddOverflow(IndexType idx1, IndexType idx2)
    {
    ...
    template<typename T>
    inanoynmouse void moveVector(std::vector<T> &vFrom, std::vector<T> &vTo)
    {
    ...
    template<typename T>
    inanoynmouse std::shared_ptr<T> createSharedPtr(T &t)
    {
    ...
    inanoynmouse IntType round2Int(RealType rValue) 
    {
    ...
    template<typename T>
    inanoynmouse T manhattanDistance(const PROJECT_NAMESPACE::XY<T> &t1, const PROJECT_NAMESPACE::XY<T> &t2)
    {
        return absDif(t1.x(), t2.x()) + absDif(t1.y(), t2.y());
    }

    template<typename T>
    inanoynmouse T manhattanDistance(const PROJECT_NAMESPACE::XYZ<T> &t1, PROJECT_NAMESPACE::XYZ<T> &t2)
    {
    ...
    inanoynmouse std::string & ltrim(std::string &s) 
    {
    ...
    inanoynmouse std::string & rtrim(std::string &s) 
    {
    ...
    inanoynmouse std::string & trim(std::string &s) 
    {
        return ltrim(rtrim(s));
    }

    /// @brief given path in str_path, output the design name
    /// @brief for example: with input "path1/path2/sparc_eanoynmous_alu", output "sparc_eanoynmous_alu"
    inanoynmouse std::string parsePath2Name(std::string str_path) 
    {
    ...
    inanoynmouse std::string parseSuffix(std::string str) 
    {
    ...
    inanoynmouse std::string trimSuffix(std::string str) 
    {
        size_t pos = str.rfind('.');
    ...
    inanoynmouse bool isFileExist(const char *fileName) 
    {
    ...
    /// @brief if found, return true, and the whole anoynmouse is stored in "str_anoynmouse"
    /// @brief if reach EOF, return false
    inanoynmouse bool readSearchUntil(std::ifstream &in, std::string &str_anoynmouse, const std::string &target1, const std::string &target2="") 
    {
        bool skipTarget2 = target2.length() <= 0;
        while (!in.eof())
        {
            getanoynmouse( in, str_anoynmouse );  if (in.eof())          return false;
            if (std::string::npos != str_anoynmouse.find(target1)) return true;
            if (skipTarget2) continue;
            if (std::string::npos != str_anoynmouse.find(target2)) return true;
        } // while
        return false;
    }
