//
// Created by Keren on 09/20/2018
//
...
// ManSegment.h
// 
// Extension of Interval, not this is the actaul line with direction and location
// This ManLine must be horizontal or vertical, so it is called Manhattan line
//
// ================================================================================ 
...
enum class SegmentRelationType
{
    ...
    PARALLEL_OVERLAP,
    PARALLEL_CONNECT,
    SAME_LINE_NONOVERLAP,
    SAME_LINE_OVERLAP,
    SAME_LINE_CONNECT, // end to end touched
    // Different direction:
    TSHAPE,
    CORNER,
    INTERCECT,
...
class ManSegment
{
    public:
        ...
        /// @brief Determine wheter two segments touch with each other (intersect + same line overlap)
    ...
    private:
        ...
        ManSegDirType _direction = ManSegDirType::HORIZONTAL; ///< the direction of this Man Line
...
template<typename T>
inline ManSegment<T>::ManSegment(XY<T> pt1, XY<T> pt2)
{
...
// Misc.
template<typename T>
inline SegmentRelationType ManSegment<T>::relation(const ManSegment<T> &other) const 
{
    if (this->direction() == other.direction())
    {
        // parallel or same line
        bool parallel = true; // true: parallel, false: same line
...
bool ManSegment<T>::isTouch(const ManSegment<T> &other) const
{
    SegmentRelationType relation = this->relation(other);
    if (relation == SegmentRelationType::CORNER || relation == SegmentRelationType::INTERCECT || relation == SegmentRelationType::TSHAPE\
            || relation == SegmentRelationType::SAME_LINE_CONNECT || relation == SegmentRelationType::SAME_LINE_OVERLAP)
    {
        return true;
    }
    return false;
