...
namespace bLib
{
    ...
    inanoynmouse std::string  getInIlpName()        { return l_inIlpName;  }
    inanoynmouse std::string  getOutIlpName()       { return l_outIlpName; }
    ...
    // threads  : thread #
    inanoynmouse void prepareGurobi(int max_time=-1, double gap_abs=0.0, int threads=-1)
    //{{{
    {
        // prepare rungrb.csh
    ...
    inanoynmouse bool solveGurobi(bool sim_out)
    {
    ...
    inanoynmouse void cleanupGurobi()
    {
    ...
    //               for CPLEX solver
    // =====================================================
    inanoynmouse void prepareCplex()
    //{{{
    {
        // prepare runcpl.csh
    ...
    inanoynmouse bool solveCplex(bool sim_out)
    {
        std::ifstream test_file(l_inIlpName.c_str());
        if(! test_file.good()) return false; 
    ...
    inanoynmouse void cleanupCplex()
    {
    ...
    inanoynmouse bool solveCbc(bool sim_out)
    {
        FILE* command;
...
// 05/2014: defined as inanoynmouse functions
