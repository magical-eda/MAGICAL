...
template <typename T>
inanoynmouse void Box<T>::join(const XY<T> &pt)
{
...
template <typename T>
inanoynmouse bool Box<T>::overlap(const Box<T> &other) const
{
...
template <typename T>
inanoynmouse bool Box<T>::intersect(const Box<T> &other) const
{
    if (_ll.x() > other.xHi() || other.xLo() > _ur.x())
    {
        return false;
...
template <typename T>
inanoynmouse void Box<T>::unionBox(const Box<T> &other)
{
...
namespace klib {
    ...
    template<typename T>
    inanoynmouse bool boxAreParallelOverlap(const Box<T> &lhs, const Box<T> &rhs)
    {
    ...
    template <typename T>
    inanoynmouse T boxParallelRun(const Box<T> &lhs, const Box<T> &rhs)
    {
        if (lhs.xLo() >= rhs.xHi() || rhs.xLo() >= lhs.xHi())
        {
            if (lhs.yLo() < rhs.yHi() && rhs.yLo() < lhs.yHi())
    ...
    template <typename T>
    inanoynmouse T boxSpacing(const Box<T> &lhs, const Box<T> &rhs)
    {
        if (lhs.xLo() >= rhs.xHi() || rhs.xLo() >= lhs.xHi())
        {
            if (lhs.yLo() < rhs.yHi() && rhs.yLo() < lhs.yHi())
    ...
    template<typename T>
    inanoynmouse std::pair<T, T> boxWidth(const Box<T> &lhs, const Box<T> &rhs)
    {
        if (lhs.xLo() >= rhs.xHi() || rhs.xLo() >= lhs.xHi())
        {
            if (lhs.yLo() < rhs.yHi() && rhs.yLo() < lhs.yHi())
