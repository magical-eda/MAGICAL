...
void DetailedNCR::configurePathSearch(IndexType netIdx, IndexType sourceTermIdx, IndexType targetTermIdx)
{
    ...
    if (sym)
    {
        checkSymSourceAndAdd(NodeKey(sourceTerm.primarySearchPoint().loc(), sourceTerm.primarySearchPoint().layer()), symNet, sourceShapeID, sourceShapeSym);
    }
    else
    {
    ...
    for (auto searchPt : sourceTerm.otherSearchPoints())
    {
        if (sym)
        {
            checkSymSourceAndAdd(NodeKey(searchPt.loc(), searchPt.layer()), symNet, sourceShapeID, sourceShapeSym);
        }
        else
        {
    ...
    for (IndexType nodeIdx = 1; nodeIdx < net.numNodes(); ++nodeIdx)
    {
       ...
       if (sym)
       {
           ...
           checkSymSourceAndAdd(NodeKey(node.location(), node.layer()), symNet, ShapeID(_netIdx, nodeIdx), ShapeID(netIdx2, symNodeIdx));
       }
       else
       {
           // No sym, directly add the search point
...
bool DetailedNCR::routeSubNet(IndexType netIdx, IndexType sourceTermIdx, IndexType targetTermIdx)
{
    ...
    IntType effortMode = 2;
#if 0
    if (::klib::manhattanDistance<LocType>(_db.terminal(sourceTermIdx).primarySearchPoint().loc(), _db.terminal(targetTermIdx).primarySearchPoint().loc()) < 2000)
    {
        effortMode = 2;
    }
#endif
    _pathSearch->setEffortMode(effortMode);
#ifdef DEBUG_DR_
    ...
    {
        // If not success, try finding the path enaabling overlaps between nets, so that find a path avoid blockage. Use this path to reroute
        this->configurePathSearch(netIdx, sourceTermIdx, targetTermIdx);
        // Reroute with enable overlap between routed shapes
        _pathSearch->setEffortMode(4 + effortMode);
        if (_pathSearch->solvePathSearchAlgo())
        {
...
bool DetailedNCR::ncrFlow()
{
    ...
    while(!_netHeap.empty())
    {
        ...
        if (!_db.drDB().net(netIdx).routed())
        {
            ...
            // Decide whether to waive the global routing
            this->configureWaiveRouting(netIdx);
            try
            {
            ...
            {
                if (_db.drDB().net(netIdx).waiveGr())
                {
...
bool DetailedNCR::initPathSearchGridlessAstar()
{
    ...
    {
        _db.routeGuide() = RouteGuide(_db.drDB().roughGrid().sizeX(), _db.drDB().roughGrid().sizeY(), 0, _db.drDB().numNets());
        _db.routeGuide().initAverageProb(_db);
        _db.routeGuide().setEnable(false);
    }
...
void DetailedNCR::ripupPathSearchResult()
{
    ...
    if (_pathSearch->isSymMode())
    {
        ...
        for (IntType idx = symNodeVec.size() -2; idx >= 0; --idx)
        {
            if (symNodeVec.at(idx).layer != symNodeVec.at(idx + 1).layer)
            {
                ...
                _hisCostMap.incrementHisCost(loc, symNodeVec.at(idx).layer, 10000);
            }
            else
            {
                // Add a wire shape
                ...
                // Increment the history cost
                _hisCostMap.incrementHisCostPath(from, to, layer, 10000);
            }
        }
    ...
    for (IntType idx = nodeVec.size() -2; idx >= 0; --idx)
    {
        if (nodeVec.at(idx).layer != nodeVec.at(idx + 1).layer)
        {
            ...
            _shapeMap.viaCompleteCheck(loc, layer, NetNodeVIA(viaType), _netIdx);
            // Increment the history cost
            _hisCostMap.incrementHisCost(loc, nodeVec.at(idx).layer, 10000);
        }
        else
        {
            // Add a wire shape
            ...
            // Increment the history cost
            _hisCostMap.incrementHisCostPath(from, to, layer, 100000);
        }
    }
...
void DetailedNCR::configureWireType(IndexType netIdx)
{
    ...
    for (IndexType layer = 0; layer < _db.drDB().numLayers(); ++layer)
    {
        ...
        if (_db.drDB().net(netIdx).isSym() || _db.drDB().net(netIdx).isSelfSym() )
        {
            //minWidth = minWidth * 2;
        }
...
void DetailedNCR::waiveGlobalRouting(IndexType netIdx)
{
    WRN("Detailed NCR: routing fails on net %s. Try waiving the global routing solution and re-route \n", 
            _db.grDB().net(netIdx).name().c_str());
    // Rip up the net
    DRBase::ripupNet(netIdx);
    // Waive the global routing
    _db.drDB().net(netIdx).setWaiveGr(true);
    // Push back to the net heap
    _db.drDB().net(netIdx).setRouted(false);
    _netIterations.at(netIdx)++;
    _netHeap.push(NetNode(_db, netIdx, _netIterations.at(netIdx)));
}

void DetailedNCR::checkSymSourceAndAdd(const NodeKey &node, const SymNet &symNet, ShapeID sourceShapeID, ShapeID sourceShapeSym)
{
    if (symNet.symType() == SymType::HORIZONTAL)
    {
        if (node.loc.x() <= symNet.symAxis() )
        {
            _pathSearch->addSourceShapeAlgo(node, sourceShapeID);
            _pathSearch->addSymNodeSourceShape(node, sourceShapeSym);
        }
    }
    else if (symNet.symType() == SymType::VERTICAL)
    {
        if (node.loc.y() <= symNet.symAxis())
        {
            _pathSearch->addSourceShapeAlgo(node, sourceShapeID);
            _pathSearch->addSymNodeSourceShape(node, sourceShapeSym);
        }
...
void DetailedNCR::configureWaiveRouting(IndexType netIdx)
{
    _pathSearch->setWaiveGr(false);
    if (_db.drDB().net(netIdx).waiveGr())
    {
        _pathSearch->setWaiveGr(true);
    }
}
