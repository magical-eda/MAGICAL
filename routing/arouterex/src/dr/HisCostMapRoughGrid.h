...
class HisCostMapRoughGrid : public HisCostMap
{
    ...
    private:
        ...
        /// @return the x index
        IndexType xIdx(LocType xLoc) const 
        { 
            if (xLoc < _offsetX) { return 0; }
            return  (xLoc - _offsetX) / _nodeWidth; 
        }
        /// @brief get the y index
        /// @return the y index
        IndexType yIdx(LocType yLoc) const { if (yLoc < _offsetY) { return 0; } return (yLoc - _offsetY) / _nodeHeight; }
    private:
...
inline HisCostMapRoughGrid::HisCostMapRoughGrid(const Box<LocType> &regionBox, LocType nodeWidth, LocType nodeHeight, IndexType numLayers)
{
...
inline CostType HisCostMapRoughGrid::hisCost(const XY<LocType> &loc, IndexType layer) const 
{
    return hisCost(loc.x(), loc.y(), layer);
}

inline CostType HisCostMapRoughGrid::hisCost(LocType x, LocType y, IndexType layer) const 
{
...
inline CostType HisCostMapRoughGrid::hisCostPath(const XY<LocType> &from, const XY<LocType> &to, IndexType layer) const
{
    CostType cost = 0;
    ...
    for (IndexType xId = xLo; xId <= std::min(xHi, _costGrid.at(layer).xSize()); ++xId)
    {
        for (IndexType yId = yLo; yId <= std::min(yHi, _costGrid.at(layer).ySize()); ++yId)
        {
            if (xId >= _costGrid.at(layer).xSize())
            {
                continue;
...
inline void HisCostMapRoughGrid::incrementHisCostPath(const XY<LocType> &from, const XY<LocType> &to, IndexType layer, CostType increment)
{
    IndexType xFrom = xIdx(from.x());
    IndexType yFrom = yIdx(from.y());
    IndexType xTo = xIdx(to.x());
...
inline void HisCostMapRoughGrid::setHisCost(const XY<LocType> &loc, IndexType layer, CostType newCost)
{
    _costGrid.at(layer).at(xIdx(loc.x()), yIdx(loc.y())) = newCost;
}

inline void HisCostMapRoughGrid::incrementHisCost(const XY<LocType> &loc, IndexType layer, CostType increment)
{
