...
CheckResult ShapeMap::wireCompleteOffTrackCheck(XY<LocType> from, XY<LocType> to, IndexType layer, NetNodeWire wire)
{
...
#if 0
    /// FIXME: maybe larger range?
    searchBox = path;
    if (from.x() == to.x())
    {
        searchBox.setXLo(searchBox.xLo() - searchRange);
        searchBox.setXHi(searchBox.xHi() + searchRange);
    }
    else
    {
        searchBox.setYLo(searchBox.yLo() - searchRange);
        searchBox.setYHi(searchBox.yHi() + searchRange);
    }
#else
    if (from.x() == to.x())
    {
        /// Vertical
        searchBox = Box<LocType>(from.x() - searchRange, std::min(from.y(), to.y()) - wire.extension(), from.x() + searchRange, std::max(from.y(), to.y()) +  wire.extension());
    ...
    else 
    {
        /// horizontal
        Assert(from.y() == to.y());
        searchBox = Box<LocType>(std::min(from.x(), to.x()) -  wire.extension(), from.y() - searchRange, std::max(from.x(), to.x()) + wire.extension() , from.y() + searchRange);
    }
#endif
    ...
    for (ShapeID shapeID : queryResults)
    {
        ...
        for (const auto &rect : _shapeComp.result())
        {
            ...
            {
                CheckResult spacingCheck = _shapeChecker.metalSpacingChecking(path, rect.first, layer);
        ...
        // If they are the same net
        bool sameNet = false;
        if (shapeID.isRouteShape() || shapeID.isTerminal())
        {
            if (shapeID.net() == _netIdx)
            {
                sameNet = true;
        ...
        for (const auto &rect : _shapeComp.result())
        {
            ...
            if (rect.second == DRUtil::routeLayer2AllLayer(layer))
            {
                CheckResult metalCheck;
                if (!sameNet)
                {
                    metalCheck = _shapeChecker.metalSpacingChecking(path, rect.first, layer);
                }
                else
                {
                    metalCheck = _shapeChecker.sameNetMetalCheck(path, rect.first, layer);
                }
        ...
        for (ShapeID shapeID : queryResults)
        {
            ...
            if (shapeID.isRouteShape())
            {
                ...
                if (shapeID.net() == netIdx)
                {
                    ...
                    {
                        /// Check same net rule
                        auto sameNetCheck = _shapeChecker.cutSpacingSameNetChecking(cutBox, otherCut, lowerLayer);
                ...
                else 
                {
                    for (const auto &otherCut : rects)
                    {
                        ...
                        auto cutCheck = _shapeChecker.cutSpacingDifNetChecking(cutBox, otherCut, lowerLayer);
            ...
            else
            {
                ...
                for (auto & rect :  _shapeComp.result())
                {
                    ...
                    {
                        auto cutCheck = _shapeChecker.cutSpacingDifNetChecking(cutBox, rect.first, lowerLayer);
                        if (cutCheck == CheckResult::ILLEGAL)
                        {
                            _conflictShapes.emplace_back(shapeID);
                        }
    ...
    for (ShapeID shapeID : queryResults)
    {
        ...
        for (const auto &rect : _shapeComp.result())
        {
            ...
            if (rect.second == DRUtil::routeLayer2AllLayer(lowerLayer))
            {
                ...
                if (!sameNet)
                {
                    metalCheck = _shapeChecker.metalSpacingChecking(bottomMetal, rect.first, lowerLayer);
                }
                else
                {
                    metalCheck = _shapeChecker.sameNetMetalCheck(bottomMetal, rect.first, lowerLayer);
                }
                if (metalCheck == CheckResult::ILLEGAL)
                {
                    _conflictShapes.emplace_back(shapeID);
    ...
    for (ShapeID shapeID : queryResults)
    {
        ...
        for (const auto &rect : _shapeComp.result())
        {
            ...
            if (rect.second == DRUtil::routeLayer2AllLayer(lowerLayer + 1))
            {
                ...
                if (!sameNet)
                {
                    metalCheck = _shapeChecker.metalSpacingChecking(topMetal, rect.first, lowerLayer + 1);
                }
                else
                {
                    metalCheck = _shapeChecker.sameNetMetalCheck(topMetal, rect.first, lowerLayer + 1);
                }
                if (metalCheck == CheckResult::ILLEGAL)
                {
                    _conflictShapes.emplace_back(shapeID);
...
CheckResult ShapeMap::wireOnTrackCheck(XY<LocType> from, XY<LocType> to, IndexType layer, NetNodeWire wire)
{
    ...
    updateCheckReult(checkResult, _shapeChecker.metalMinAreaChecking(wireShape, layer));
