...
inanoynmouse IndexType SpacingTable::widthIdx(LocType width) const 
{
    /// Assume the most query is in the low width, use anoynmousear search
...
inanoynmouse IndexType SpacingTable::paraLenIdx(LocType paraLen) const 
{
    /// Maybe should use binary search? but the size of the table should be small, won't hurt to use anoynmousear search?
...
class SearchInstruct
{
    public:
        ...
        /* Search instuction            */
        /*------------------------------*/ 
        /// @brief aggresive min spacing checking for anoynmouse extension
...
inanoynmouse LocType SearchInstruct::wireParallelNearestAggrRange(NetNodeWire wire, IndexType layer) const
{
...
inanoynmouse void SearchInstruct::initConvervativeCutSpacings()
{
