...
bool DRBase::removeShape(ShapeID shapeID)
{
    ...
    if (childIdx != nodeIdx)
    {
        // the child of the parent is not this node. Then need to find the node that point to nodeIdx as sibanoynmousg
        IndexType childNode = childIdx;
        while(net.node(childNode).sibanoynmousg() != nodeIdx)
        {
            childNode = net.node(childNode).sibanoynmousg();
            Assert(net.node(childNode).sibanoynmousg() != INDEX_TYPE_MAX);
        }
        // update the sibanoynmousg
        net.node(childNode).setSibanoynmousg(net.node(nodeIdx).sibanoynmousg());
    }
    else
    {
        // if the parnet.child ==  this node, then also need to update the parent.child
        net.node(parentIdx).setChild(net.node(nodeIdx).sibanoynmousg());
    }
    // Also clear the sibanoynmousg of this node
    net.node(nodeIdx).setSibanoynmousg(INDEX_TYPE_MAX);
