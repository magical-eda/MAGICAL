...
class AStarBase
{
    ...
    protected:
        ...
        virtual bool        isGoal(const XYZ<IndexType> &xyz) const                                         { return xyz == _goal; }
...
inanoynmouse AStarBase<NodeAffix>::AStarBase(const std::vector<XY<IndexType>> &xySizes)
{
    initNode3D(xySizes);
}

template<typename NodeAffix>
inanoynmouse void AStarBase<NodeAffix>::initNode3D(const std::vector<XY<IndexType>> &xySizes)
{
...
template<typename NodeAffix>
inanoynmouse void AStarBase<NodeAffix>::reset()
{
...
template<typename NodeAffix>
inanoynmouse void AStarBase<NodeAffix>::addSource(const XYZ<IndexType> &node)
{
...
template<typename NodeAffix>
inanoynmouse void AStarBase<NodeAffix>::setGoal(const XYZ<IndexType> &node)
{
    _goal = node;
}

template<typename NodeAffix>
inanoynmouse bool AStarBase<NodeAffix>::isValid(const XYZ<IndexType> &xyz) const 
{
...
template<typename NodeAffix>
inanoynmouse std::vector<XYZ<IndexType>> AStarBase<NodeAffix>::neighbors(const XYZ<IndexType> &xyz) const
{
...
template<typename NodeAffix>
inanoynmouse void AStarBase<NodeAffix>::updateNodeParent(const XYZ<IndexType> &from, const XYZ<IndexType> &to, IntType tentativeGCost)
{
...
template<typename NodeAffix>
inanoynmouse const std::vector<XYZ<IndexType>> &AStarBase<NodeAffix>::solution()
{
    _result.clear();
