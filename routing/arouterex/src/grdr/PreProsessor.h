...
/// @class PROJECT_NAMESPACE::ProcessBlockage
/// @brief process the blockage-related things
class ProcessBlockage
{
    public:
        /// @brief Constructor
        /// @param The database for the router
        explicit ProcessBlockage(Database &db) : _db(db) {}
        /// @brief process the blockages
        /// @return if successful
        bool process();
    private:
        /// @brief Corelate the shapes read from the gds with the pin shapes and removed the shapes that are actually not blockages
        bool mergePins();
    private:
        Database &_db; ///< The database for the router
};
