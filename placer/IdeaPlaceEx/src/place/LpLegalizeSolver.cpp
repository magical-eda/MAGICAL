    ...
    if (_optHpwl)
    {
        for (IndexType netIdx = 0; netIdx < _db.numNets(); ++netIdx)
        {
            ...
            for (IndexType pinIdxInNet =0; pinIdxInNet < net.numPinIdx(); ++pinIdxInNet)
            {
                ...
                if (_isHor)
                {
                    _ilpModel.addConstraint( 2 * AT(_wlL, netIdx)
                            - AT(_locs, pin.cellIdx()) * 2
                            <=  _db.cell(pin.cellIdx()).cellBBox().xLen());
                    // wl_r >= _loc + pin_offset for all pins in the net
                    _ilpModel.addConstraint( 2 * AT(_wlR, netIdx)
                            - 2*  AT(_locs, pin.cellIdx()) 
                            >=  _db.cell(pin.cellIdx()).cellBBox().xLen());
                }
                else
                {
                    ...
                    _ilpModel.addConstraint( 2 * AT(_wlL, netIdx)
                            - 2 * AT(_locs, pin.cellIdx())
                            <=  _db.cell(pin.cellIdx()).cellBBox().yLen() );
                    // wl_r >= _loc + pin_offset for all pins in the net
                    _ilpModel.addConstraint(  AT(_wlR, netIdx) * 2
                            - AT(_locs, pin.cellIdx() ) * 2 
                            >=   _db.cell(pin.cellIdx()).cellBBox().yLen() );
                }
            }
