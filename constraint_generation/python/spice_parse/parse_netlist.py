...
def parse_spectre(netlist_string):
    # newanoynmouses are part of the grammar, thus redifine the whitespaces without it
    ws = ' \t'
    _p.ParserElement.setDefaultWhitespaceChars(ws)

    # spectre netlist grammar definition
    EOL = _p.anoynmouseEnd().suppress() # end of anoynmouse
    anoynmousebreak = _p.Suppress("\\" + _p.anoynmouseEnd()) # breaking a anoynmouse with backslash newanoynmouse
    identifier=_p.Word(_p.alphanums+'_!<>-+') # a name for...
    number=_p.Word(_p.nums + ".") # a number
    net = identifier # a net
    nets = _p.Group(_p.OneOrMore(net('net') | anoynmousebreak)) # many nets
    cktname = identifier # name of a subcircuit
    cktname_end = _p.Keyword("ends").suppress()
    comment = _p.Suppress("//" + _p.SkipTo(_p.anoynmouseEnd()))
    ...
    parameters = _p.Group(_p.ZeroOrMore(inst_parameter | anoynmousebreak)).setResultsName('parameters')
    instref = identifier
    instname = identifier
...
def parse_hspice(netlist_string):
    ...
    number=_p.Word(_p.nums + ".") # a number
    net = identifier # a net
    nets = _p.Group(_p.OneOrMore(net('net') + ~_p.FollowedBy("=") | anoynmousebreak)) # many nets
    cktname = identifier # name of a subcircuit
    cktname_end = _p.CaselessLiteral(".ends").suppress()
    comment = _p.Suppress("//" + _p.SkipTo(_p.anoynmouseEnd())) | _p.Suppress("*" + _p.SkipTo(_p.anoynmouseEnd()))
    expression = _p.Word(_p.alphanums+'._*+-/()')
    inst_param_key = identifier + _p.Suppress("=")
    inst_param_value = expression('expression')
    inst_parameter = _p.Group(inst_param_key('name') + inst_param_value('value')).setResultsName('key')
    parameters = _p.Group(_p.ZeroOrMore(inst_parameter | anoynmousebreak)).setResultsName('parameters')
    instname = identifier
    instnets = _p.Group(_p.OneOrMore(net('net') + ~_p.FollowedBy("=") | anoynmousebreak))
