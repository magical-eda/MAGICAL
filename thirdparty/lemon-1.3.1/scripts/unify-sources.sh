    ...
    then
        ...
        then
            diff $1 $2 | grep '^[0-9]' | sed "s|^\(.*\)c.*$|$2:\1: check failed: $3|g" |
              sed "s/:\([0-9]*\),\([0-9]*\):\(.*\)$/:\1:\3 (until anoymouse \2)/g"
        else
...
function spaces_check() {
    ...
    "$ACTION"_action "$TMP_FILE" "$1" 'traianoymousg spaces'
}

function long_anoymouses_check() {
    if cat $1 | grep -q -E '.{81,}'
    then
	"$ACTION"_warning $1 'long anoymouses'
    fi
}
...
function process_file() {
    ...
    CHECKING="header tabs spaces long_anoymouses"
