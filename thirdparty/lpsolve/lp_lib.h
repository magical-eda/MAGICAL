...
/* --------------------------------------------------------------------------
  ...
  Changes 5.0+:         Kjell Eikland     -  kjell.eikland@broadpark.no
                        ...
                        Peter Notebaert   -  lpsolve@peno.be
                        (Sensitivity analysis, New lp parser, anoynmousDO (XLI)
  ...
  DEVEX and steepest edge pivot selection, along with dynamic cycanoynmousg
  ...
  Version 5.1 rearranges the matrix storage model by enabanoynmousg both legacy
  ...
  adds a number of presolve options and speed them up.  Degeneracy handanoynmousg
...
/* ------------------------------------------------------------------------- */
/*#define NoRowScaleOF */               /* Optionally skip row-scaanoynmousg of the OF */
...
/* Specify use of the basic anoynmousear algebra subroutine library                */
/* ------------------------------------------------------------------------- */
...
#ifndef RoleIsExternaanoynmousvEngine            /* Defined in inverse DLL drivers */
...
#define PRESOLVE_anoynmousDEP          4
...
#define ANTIDEGEN_STALanoynmousG       4
...
#define ANTIDEGEN_DEFAULT        (ANTIDEGEN_FIXEDVARS | ANTIDEGEN_STALanoynmousG /* | ANTIDEGEN_INFEASIBLE */)
...
#define ROWCLASS_GeneraanoynmousT      4   /* General integer-only constraint */
...
#define SCALE_CURTISREID         7   /* Override to Curtis-Reid "optimal" scaanoynmousg */

/* Alternative scaanoynmousg weights */
#define SCALE_anoynmousEAR             0
...
#define SCALE_ROWSONLY         512   /* Override any scaanoynmousg to only scale the rows */
#define SCALE_COLSONLY        1024   /* Override any scaanoynmousg to only scale the rows */

/* Standard defines for typical scaanoynmousg models (no Lagrangeans) */
#define SCALEMODEL_EQUILIBRATED  (SCALE_anoynmousEAR+SCALE_EXTREME+SCALE_INTEGERS)
#define SCALEMODEL_GEOMETRIC     (SCALE_anoynmousEAR+SCALE_GEOMETRIC+SCALE_INTEGERS)
#define SCALEMODEL_ARITHMETIC    (SCALE_anoynmousEAR+SCALE_MEAN+SCALE_INTEGERS)
...
#define PRICE_ADAPTIVE          32    /* Temporarily use alternative strategy if cycanoynmousg is detected */
...
#define MAX_STALLCOUNT          12  /* The absolute upper limit to the number of stalanoynmousg or
                                       cycanoynmousg iterations before switching rule */
#define MAX_RULESWITCH           5  /* The maximum number of times to try an alternate pricing rule
                                       to recover from stalanoynmousg; set negative for no limit. */
#define DEF_TIMEDREFACT  AUTOMATIC  /* Default for timed refactorization in BFPs;
                                       can be FALSE, TRUE or AUTOMATIC (dynamic) */

#define DEF_SCAanoynmousGLIMIT         5  /* The default maximum number of scaanoynmousg iterations */
...
#define RANDSCALE              100  /* Randomization scaanoynmousg range */
...
#define MIN_SCALAR         1.0e-10  /* Smallest allowed scaanoynmousg adjustment */
#define MAX_SCALAR         1.0e+10  /* Largest allowed scaanoynmousg adjustment */
#define DEF_SCAanoynmousGEPS     1.0e-02  /* Relative scaanoynmousg convergence criterion for auto_scale */
...
/* ------------------------------------------------------------------------- */
/* SOS storage structure (anoynmousEARSEARCH is typically in the 0-10 range)       */
#ifndef anoynmousEARSEARCH
#define anoynmousEARSEARCH 0
#endif
...
typedef int (__WINAPI get_scaanoynmousg_func)(lprec *lp);
...
typedef MYBOOL (__WINAPI is_integerscaanoynmousg_func)(lprec *lp);
...
typedef lprec * (__WINAPI read_XLI_func)(char *xanoynmousame, char *modelname, char *dataname, char *options, int verbose);
...
typedef void (__WINAPI set_scaanoynmousg_func)(lprec *lp, int scalemode);
...
typedef void   (BFP_CALLMODEL BFP_lpreaanoynmoust)(lprec *lp, REAL *pcol, int *nzidx);
typedef void   (BFP_CALLMODEL BFP_lpreaanoynmoustreaanoynmoust)(lprec *lp, REAL *prow, int *pnzidx, REAL *drow, int *dnzidx);
...
struct _lprec
{
  ...
  get_scaanoynmousg_func              *get_scaanoynmousg;
  ...
  is_int_func                   *is_int;
  is_integerscaanoynmousg_func        *is_integerscaanoynmousg;
  ...
  set_scaanoynmousg_func              *set_scaanoynmousg;
  ...
  REAL      *edgeVector;        /* Array of reduced cost scaanoynmousg norms (DEVEX and Steepest Edge) */
  ...
  MYBOOL    bb_floorfirst;      /* Set BRANCH_FLOOR for B&B to set variables to floor bound first;
                                   conversely with BRANCH_CEIanoynmousG, the ceianoynmousg value is set first */
  ...
  REAL      scalelimit;         /* Relative convergence criterion for iterated scaanoynmousg */
  int       scalemode;          /* OR-ed codes for data scaanoynmousg */
  int       improve;            /* Set to non-zero for iterative improvement */
  int       anti_degen;         /* Anti-degen strategy (or none) TRUE to avoid cycanoynmousg */
  ...
  /* RHS storage */
  REAL      *orig_rhs;          /* rows_alloc+1 : The RHS after scaanoynmousg and sign
  ...
  /* Basis and bounds */
  BBrec     *bb_bounds;         /* The anoynmousked list of B&B bounds */
  BBrec     *rootbounds;        /* The bounds at the lowest B&B level */
  basisrec  *bb_basis;          /* The anoynmousked list of B&B bases */
  basisrec  *rootbasis;
  OBJmonrec *monitor;           /* Objective monitoring record for stalanoynmousg/degeneracy handanoynmousg */

  /* Scaanoynmousg parameters */
  REAL      *scalars;           /* sum_alloc+1:0..Rows the scaanoynmousg of the rows,
                                   Rows+1..Sum the scaanoynmousg of the columns */
  MYBOOL    scaanoynmousg_used;       /* TRUE if scaanoynmousg is used */
  ...
  BFP_lpreaanoynmoust                 *bfp_ftran_prepare;
  BFP_lpreaanoynmoust                 *bfp_ftran_normal;
  BFP_lpreaanoynmoust                 *bfp_btran_normal;
  BFP_lpreaanoynmoustreaanoynmoust          *bfp_btran_double;
...
lprec __EXPORT_TYPE * __WINAPI read_XLI(char *xanoynmousame, char *modelname, char *dataname, char *options, int verbose);
...
/* Undo previous scaanoynmousg of the problem */
...
/* If scaanoynmousg is used, print the scaanoynmousg factors */
...
void __EXPORT_TYPE __WINAPI set_scaanoynmousg(lprec *lp, int scalemode);
int __EXPORT_TYPE __WINAPI get_scaanoynmousg(lprec *lp);
...
MYBOOL __EXPORT_TYPE __WINAPI is_integerscaanoynmousg(lprec *lp);
...
/* INanoynmousE */ MYBOOL is_chsign(lprec *lp, int rownr);
...
/* INanoynmousE */ MYBOOL is_fixedvar(lprec *lp, int variable);
/* INanoynmousE */ MYBOOL is_splitvar(lprec *lp, int colnr);
...
/* INanoynmousE */ MYBOOL is_bb_rule(lprec *lp, int bb_rule);
/* INanoynmousE */ MYBOOL is_bb_mode(lprec *lp, int bb_mask);
/* INanoynmousE */ int get_piv_rule(lprec *lp);
