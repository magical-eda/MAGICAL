...
local void lo_init_substitute_alloc(void)
{
  ...
  ** redefines malloc */
  ...
  lo_substitute_alloc_buf =
  /**/		(char *) lo_really_malloc(LO_SUBSTITUTE_ALLOC_BUF_SIZE);
...
local void test_mem_detailed(void)
{
  ...
  wn_memgp child1, child2, child3, child4;
  int start_mem_used;
  int i, j, k;
...
# define FREEABLE_GROUP(k)  (!(k & 1))
  ...
  for (k = 0;  k < (int) (sizeof(group_types) / sizeof(group_types[0]));  ++k)
  {
	...
	wn_memset(mem_array1[j], j % 253, j);
      }
	...
	wn_assert(wn_memtest(mem_array1[j], j % 253, j));
      }
	...
	wn_memset(mem_array2[j], j % 221, j);
      }
	...
	wn_assert(wn_memtest(mem_array2[j], j % 221, j));
      }
	...
	mem_array1[j] = wn_alloc(j);
	wn_memset(mem_array1[j], j % 193, j);
      }
	...
	wn_assert(wn_memtest(mem_array1[j], j % 193, j));
      }
	...
	for (i = 0;  i < 1000;  ++i)
	{
	  ...
	  wn_assert(!mem_array1[j]);
	  mem_array1[j] = wn_alloc(j);
	  wn_memset(mem_array1[j], j % 157, j);
	}
	...
	for (i = 0;  i < 1000;  ++i)
	{
	  ...
	  wn_assert(wn_memtest(mem_array1[j], j % 157, j));
	}
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "testing mems version of memory stuff%s%s%s ...\n",
  /**/		fill ? " -fill" : "",
