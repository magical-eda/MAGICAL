...
local unsigned lo_ciel(unsigned i, unsigned mod) {
  unsigned j;
...
/*****************************************************************
**  
**  To generate a table of random ints, say "examples -true 1 <#>"
**  where <#> is the number of random ints you want in the table.
**  This will generate c code for a global table to stdout.  Save
**  it to a file, edit the declaration (maybe make it local, change
**  the name ...) and you're ready to compile it.
**
*****************************************************************/

#if !defined(WN_WINDOWS)
local void lo_test_generate_true_random_table(int size) {
  int i, j;
  char buf[100];

  printf("int true_random_int_table[] = {");
  for (i = 0;  i < size;  ++i) {
    if (!(i % 100))
    {
      j = wn_min(i+19, size-1);
      printf("%s\n  /****** %d to %d ******/\n  ", i ? ",\n\n" : "" , i, j);
    }
    else if (!(i % 20))
    {
    ...
    sprintf(buf, "0x%x", wn_true_random_int());
    printf("(int) %10s", buf);
  }
  printf("\n};\n");
} /* lo_test_generate_random_table */
#endif /* !WN_WINDOWS */
...
int main(int argc,char *argv[])
{
  ...
  static bool arg_true; /* generate table of 'trials' true random ints */
  static char *bool_names[] = { "FALSE", "TRUE" };
  int trials, random_seed_arg;
  int sts, i;

  /*     note we can't use the arg parsing package here because arg is above
  ** random */

  if (argc < 2)
  {
    fprintf(stderr,
    /**/  "Usage: examples -rndt [-true <sizeof table>] <random seed>\n"
    /**/					" <trials = 726817>\n%s",
    /**/  "  where exactly one of d, g, n, r, s, t, or true must be set\n");
    return 1;
  }
  ...
  if (arg_c + arg_d + arg_g + arg_n + arg_p + arg_r + arg_s + arg_t + arg_true
  /**/									!=  1)
  {
    fprintf(stderr,
    /**/  "Usage: examples -rndt [-true <sizeof table>] <random seed>\n"
...
#   if !defined(WN_WINDOWS)
      wn_assert(arg_true);
      lo_test_generate_true_random_table(trials);
#   else
      fprintf(stderr, "True random numbers not supported on Windows\n");
      exit(1);
#   endif
  }
