...
#if BOGDAN
  bool min_lost; /* true if a minimum has been thrown away while shrinking bracketing interval */
...
typedef struct context_struct
{
  ...
  bool isreal1;
  bool isrealls;
  double norm2_grad0,norm2_grad1;
...
local int fdcmp(double f1,double df1,double f2,double df2)
{
  ...
  else if(f1 < f2) /* &&(!(fabs(f1 - f2) < pow(2.0, -52.0)*.5*(fabs(f1) + fabs(f2)))) */
  {
    return(-1);
  }
  else  /* f's equal, look at df's */
  {
    adf1 = wn_clip_f(wn_abs(df1));
    adf2 = wn_clip_f(wn_abs(df2));
			...
			double coef[4])
{
  ...
  w=-0.5*(d_coef[1]+wn_sign(d_coef[1])*sqrt_discriminant);
  if(!(w != 0.0))
  {
    *psuccess = FALSE;
    return;
...
{
  /* jump len is exponentially declining weighted average of
     past jump lengths */

  c->x0s = -signed_len(c->dot_grad0_dir0,c->step_len0);
  if((!(c->x0s != 0.0))||(c->iters_since_best_f0 > 6))
  {
    c->x0s = wn_random_double_between(0.5,1.5)*(c->x0s_init);
  }
...
#   endif
  ...
  else /* f0s not enough improvement */
...
#   endif
    ...
    /* f0opt worse than f0s, swap them */
...
#     endif
    ...
    if(opt_improvement_adequate(c->x0opt,c->f0opt,c->dot_grad0opt_dir0,
			        0.0,c->f0,c->dot_grad0_dir0,
			        0.75))
    {
#     if DEBUG
...
)
{
  ...
  *psubstantial_change = FALSE;
  ...
  if(cmp <= 0)  /* new point is best */
  {
    ...
    {
#if BOGDAN
      if(c->sls.isreal0 && c->sls.isreal1&&min_is_bracketed(c->sls.x1,c->sls.f1,c->sls.df1,c->sls.x0,c->sls.f0,c->sls.df0))
	{
#if DEBUGBC
    ...
    {
#if BOGDAN
      if(c->sls.isreal2 && c->sls.isreal1&&min_is_bracketed(c->sls.x1,c->sls.f1,c->sls.df1,c->sls.x2,c->sls.f2,c->sls.df2))
	{
#if DEBUGBC
  ...
  else /* !(cmp <= 0)  new point NOT is best */
  {
    if(x < c->sls.x1)
    {
#if BOGDAN
      if(c->sls.isreal0 && min_is_bracketed(x,f,df,c->sls.x0,c->sls.f0,c->sls.df0))
	{
#if DEBUGBC
    ...
    {
#if BOGDAN
      if(c->sls.isreal2 && min_is_bracketed(x,f,df,c->sls.x2,c->sls.f2,c->sls.df2))
	{
#if DEBUGBC
...
#if BOGDAN
      if(!(c->sls.f2>c->sls.fstart)&&!(c->sls.f1>c->sls.fstart))
	c->sls.bisection_count += 1;
#endif
...
#if BOGDAN
      if(!(c->sls.f0>c->sls.fstart)&&!(c->sls.f1>c->sls.fstart))
	c->sls.bisection_count += 1;
#endif
...
#if BOGDAN
  c->sls.min_lost = 0; /* no minimum has been lost through bracketing shrinking */
  c->sls.bisection_count = 0; /* no robust probes have been generated yet */
#endif
...
#   endif
    ...
    if(robust)
    {
      gen_robust_probe(c,&success,&xnew);
      quadconv = FALSE;
    }
    else
    {
...
#   if DEBUG
      printf("do_slow_linesearch: xnew=%20.100lg,fnew=%20.100lg,dfnew=%20.20lg\n",
	     xnew,fnew,dfnew);
#   endif 
...
#if BOGDAN 
    /* a termination condition less vulnerable to noise */
    if(
       c->sls.min_lost
       &&
...
#if DEBUG2D
    printf("We are at [ %20.100lg %20.100lg ]\n", c->vect0[0], c->vect0[1]);
    printf("New direction is [ %20.100lg %20.100lg ]\n", c->dir0[0], c->dir0[1]);
#endif
