...
int main(int argc, char *argv[])
{
  ...
  for (i = 1;  i < argc;  ++i)
  {
    ...
    {
      ++i;
      sts = sscanf(argv[i], "%x", &arg_trap_address_address);
      wn_assert(sts == 1);
    ...
    {
      ++i;
      sts = sscanf(argv[i], "%x", &arg_mem_in_group_address);
      wn_assert(sts == 1);
    }
    ...
    {
      ++i;
      sts = sscanf(argv[i], "%x", &arg_r_mem_to_read_from);
      wn_assert(sts == 1);
    }
    else if (!strcmp("-w", argv[i]))
    {
      ++i;
      sts = sscanf(argv[i], "%x", &arg_w_mem_to_write_to);
      wn_assert(sts == 1);
    }
    else if (!strcmp("-arrayread", argv[i]))
    {
      ++i;
      sts = sscanf(argv[i], "%x", &arg_array_address);
      wn_assert(sts == 1);

      ++i;
      sts = sscanf(argv[i], "%x", &arg_array_index);
      wn_assert(sts == 1);
    }
  ...
  p = malloc(31);	/* make a lost segment for valgrind to notice */
  p = NULL;
