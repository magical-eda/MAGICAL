...
map<string, double> NLPlace::net_wgts;
...
//{{{
{
	...
	// wire length penalty
	// net weights
	map<string, double> net_wgts = NLPlace::net_wgts;
	for (auto it=NLPlace::m_en_vids.begin(); it!=NLPlace::m_en_vids.end(); ++it) {
		string net_name = it->first;
		double net_wgt = net_wgts.at(net_name);
		result += net_wgt * lambda3 * log_sum_exp_vec(values, it->second, alpha);
	}
...
//{{{
{
	...
	for (auto it=NLPlace::m_en_vids.begin(); it!=NLPlace::m_en_vids.end(); ++it) {
		...
		for (size_t i : it->second) {
			grad[2*i] += net_wgt * lambda3 * grad_log_sum_exp_vec(values, it->second, alpha, i, 1); // x coordinate
			grad[2*i+1] += net_wgt * lambda3 * grad_log_sum_exp_vec(values, it->second, alpha, i, 0); // y coordinate
		}
	}
...
//{{{
{
	...
	std::string line;
	while(std::getline(params_in_file, line)) {
	    std::istringstream iss(line);
	    string param_name;
		iss >> param_name;
		if(param_name=="SCALING_FACTOR") {
			double sf;
			while(iss >> sf) {
				M_DEBUG(
				cout << "read in scaling factor " << sf << endl;
				);
				sfs.push_back(sf);
			}
		} 
...
//{{{
{
	...
	in_net_wgt_file_name = files_path+"/"+ckt_name+".netwgt";
...
void NLPlace::read_net_weight_file()
//{{{
{
	ifstream in_net_file;
	in_net_file.open(in_net_wgt_file_name);
    if (!in_net_file.good())
    {
        cout << "failed to open " << in_net_wgt_file_name << " for read, set to default net weights\n";
        // exit(-1);
		// initialize default net weight to be 1
	    for (auto eit : m_en_vids) {
			string net_name = eit.first;
			net_wgts[net_name] = 1.0;
	    }
    }
	...
	while(std::getline(in_net_file, line)) {
		...
		if(m_en_vids.find(net_name) != m_en_vids.end()) {
			net_wgts[net_name] = net_wgt;
		} else {
			cout << "net " << net_name << " has only one pin, ignore" << endl;
		}
	}

	in_net_file.close();
	M_DEBUG(
	cout<<"finish reading net weight file"<<endl;
	);
}
//}}}
...
//{{{
{
	...
	std::string line;
	bool cap_flag = false;
	while(std::getline(in_overlap_file, line)) {
	    std::istringstream iss(line);
		string a = "";
		while (iss >> a) {
...
//{{{
{
	...
	for (Edge e : edge_vec) {
		...
		for(auto it=vtcs[src].feol_layers.begin(); it!=vtcs[src].feol_layers.end(); ++it) {
		    ...
		    for(auto it2=vtcs[tgt].feol_layers.begin(); it2!=vtcs[tgt].feol_layers.end(); ++it2) {
				...
				if(layer_name.at(0) != 'M' && layer_id1 == layer_id2) continue; // ignore the layer spacing except the metal layers
				double xl;
	...
	if (opt_hpwl) {
	    ...
	    for (auto eit : m_en_vids) {
			...
			double net_wgt = net_wgts.at(net_name);
			// add objectives
            obj += net_wgt * (wlr[net_cnt]-wll[net_cnt]); 
	    	++net_cnt;
	    }
