...
public:
	...
	Device(string n, size_t i) : name(n), index(i) {}
	Device(string n, T x, T y, T w, T h) : name(n) {
		loc = Loc<T>(x, y);
		size = Loc<T>(w, h);
	}
	Device(string n, size_t i, int t, Loc<T> const& l, Loc<T> const& s, Loc<T> const& o, vector< LayerShape > const& lsh, GdsCell* c) : name(n), index(i), type(t), loc(l), size(s), offset(o), layers_shapes(lsh), gds_cell(c) {}
	string name;
	size_t index;
...
public:
	...
	void align_to_grid_legalize(polygon_90_t const& contour, point_t const& point, set<size_t> const& dset, polygon_90_t& aligned_contour, vector< polygon_90_t >&);
	...
	void get_rect_vec_envelope(rectangle_t& envelope, vector< rectangle_t > const& rect_vec);
	void get_polygon_set_envelope(rectangle_t&, polygon_90_set_t const&);
	void get_polygon_envelope(rectangle_t&, polygon_90_t const&);
	...
	*/
	...
	void legalize_well_rectangular(vector< polygon_90_t > const& legalized_contours, vector< set<size_t> > const& devices_in_well, vector< polygon_90_t >& rect_contours, vector< set<size_t> >& devices_in_well_rect);
	...
	void legalize_well(vector< polygon_90_t > const& legalized_contours, vector< set<size_t> > const& devices_in_well);
	void init_individual_well(vector< polygon_90_t > & rect_contours, vector< set<size_t> > & devices_in_well_rect);
	void rect2poly90(rectangle_t const& r, polygon_90_t & p);
	...
	void get_edges_between_wells(vector< dEdge >& edges, vector< polygon_90_t > const& legalized_contours);
	void get_edges_between_two_wells(vector< dEdge >& edges, polygon_90_t const& p1, point_t const& orig1, size_t well_id1, polygon_90_t const& p2, point_t const& orig2, size_t well_id2);
	void get_edges_rectangular(vector< dEdge >& edges, vector< Device<data_type> > const& boxes);
	void get_spacings(vector<int>& spacings, size_t device_id);
	void get_spacings_between(vector<int>& spacings, Device<data_type> const& d1, Device<data_type> const& d2);
	void legalize_lp(vector< dEdge > const& well_edges, vector< dEdge > const& device_edges, vector< polygon_90_t > const& legalized_contours, vector<int>& x_values, vector<int>& y_values, vector< pair<int, int> >& well_disp, vector< set<size_t> > const& devices_in_well, vector< Device<data_type> > const& boxes);
	...
	void write_nwell_guard_ring_to_file(vector< vector<point_type> > const& out_polys, vector< pair<int, int> > const& well_disp, string out_file_name);
	...
	void write_gds_cell(GdsParser::GdsDB::GdsDB* db, vector<int> const& x_values, vector<int> const& y_values, vector< Device<data_type> > & devices);
	void write_gds_well_cell(GdsParser::GdsDB::GdsDB* db, vector< pair<int, int> > const& well_disp, vector< Device<data_type> > const& boxes);
	...
	void write_gds_guard_ring(GdsParser::GdsDB::GdsDB const* db, GdsCell* cell, string out_file_name, bool nwell, vector<point_type> & out_poly);
	GdsCell* construct_cell_for_well(GdsParser::GdsDB::GdsDB* db, polygon_90_t const& p, int nw_layer, int datatype, string well_name, set<size_t> const& dset);
	void get_well_device_offset(GdsParser::GdsDB::GdsDB const* well_db, GdsCell * it, Device<data_type> & device);
	void construct_device_for_well(GdsParser::GdsDB::GdsDB const* well_db, GdsCell * it, Device<data_type> & device);
	void construct_boxes_for_wells(vector< Device<data_type> >& boxes, GdsParser::GdsDB::GdsDB* well_db, vector< polygon_90_t > const& legalized_contours, vector< set<size_t> > const& devices_in_well, vector< vector<point_type> > & out_polys);
