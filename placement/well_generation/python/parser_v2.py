...
#add='/home/polaris/yibolin/Documents/Projects/utda-toshiba/SRAF_insertion/SRAF_xiaoqing/gds/initial/MB_SRAF_clips/'
#add='/home/local/eda03/yibolin/documents/Projects/resistmodel/results/N10/'
...
def read(infile,add='', desired_size=256):
    ...
    if (3,0) in inPolygons:                      # this is only to check if the layer we are interested in is present in the gds
    	for pp in inPolygons[(3,0)]:             # read all polygons corresponding to the desired layer (2010) in this case
    	    ...
    	    ImageDraw.Draw(img_b).polygon(pa.ravel().tolist(), outline=(0,0,255), fill=(0,0,255)) #draw the polygon on the image . . fill (R,G,B) is the color scheme . outline is similar
    ...
    if (6,0) in inPolygons:                     
    	for pp in inPolygons[(6,0)]:             
    	    ...
    	    ImageDraw.Draw(img_g).polygon(pa.ravel().tolist(), outline=(0,255,0), fill=(0,255,0)) 
    ...
    if (6,1) in inPolygons:                     
    	for pp in inPolygons[(6,1)]:             
    	    ...
    	    pa[:,1] -= bbox_y[0]
    	    pa = (pa/scale).astype(int)         
    	    ImageDraw.Draw(img_r).polygon(pa.ravel().tolist(), outline=(255,0,0), fill=(255,0,0)) 
...
def main(argv):
    ...
    # add="/home/local/eda06/biyingxu/Research/idea_place/WellGen/DataWellGen2/"
    read(fname, add)
