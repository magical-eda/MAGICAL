//
// Created by Keren on 05/02/2018
//
...
namespace klib 
{
    ...
    class MSTSolver 
    {   
        inline std::string mSTSolverMethodTypeToStr(MSTSolverMethodType algorithm) 
        {
            switch(algorithm) 
            {
    ...
    /// Constrcutor
    inline MSTSolver::MSTSolver(MSTSolverMethodType algorithm, bool directed) 
    {
    ...
    /// Build the graph
    inline void MSTSolver::addVertex() 
    {
    ...
    inline void MSTSolver::addEdge(IndexType fromIdx, IndexType toIdx) 
    {
        if (_algorithm == MSTSolverMethodType::BOOST_KRUSKAL) 
        {
            if (_directed) 
    ...
    inline void MSTSolver::addEdge(IndexType fromIdx, IndexType toIdx, IntType edgeWeight) 
    {
        if (_algorithm == MSTSolverMethodType::BOOST_KRUSKAL) 
        {
            if (_directed) 
    ...
    inline std::vector<std::pair<IndexType, IndexType>> MSTSolver::resultVertexPairs() 
    {
